...
TH1D *filter_and_filling(TH1D *histopointer, int particle_monto_carlo_number, unsigned iterator, vector<int> *some_vector1, vector<Double_t> *some_vector2)
{

	if(some_vector1->at(iterator) == particle_monto_carlo_number)
	{
		histopointer->Fill(some_vector2->at(iterator));
	}
	return histopointer;
...
TH1D *filter_pointer(double latitud_mini, double latitud_maxi, double longitud_mini, double longitude_maxi, vector<Double_t>* latitude_vec, vector<Double_t>* longitude_vec,vector<int>* PDG_vector, vector<Double_t>* energy_vector,
   int particle_monto_carlo_number, TH1D *pointer1, TH1D *pointer2, TH1D *pointer3)
{
	...
	for (unsigned index =0; index < longitude_vec->size(); index++){
		 ...
		 (longitud_mini < longitude_vec->at(index) && longitude_vec->at(index)< longitude_maxi))
		{
			...
			filter_and_filling(pointer1, -particle_monto_carlo_number, index, PDG_vector, energy_vector);
			filter_and_filling(pointer2, -particle_monto_carlo_number, index, PDG_vector, energy_vector);
			filter_and_filling(pointer3, particle_monto_carlo_number, index, PDG_vector, energy_vector);
...
Double_t *ratio(int bin_numbers, Double_t *flux_data)
{
	Double_t *ratio;
	for(int n = 0; n<bin_numbers; n++)
	{
...
TGraph *create_graph(TString title,int bin_numbers, axis ax, Double_t *yaxis,Float_t color)
{
	TGraph *graph = new TGraph(bin_numbers, ax.axis_array, yaxis);
...
void draw_two_graphs(TString name, TString title,TString graph_title1, TString graph_title2, int bin_numbers, axis ax, Double_t *yaxis, Double_t *yaxis2, Float_t color, Float_t color2) 
{
	TMultiGraph *multipointer = new TMultiGraph();
	multipointer->SetNameTitle(name, title);
	TGraph *graph_pointer = create_graph(graph_title1, bin_numbers, ax, yaxis, color);
	graph_pointer->Draw("P");
	TGraph *graph_pointer2 = create_graph(graph_title2, bin_numbers, ax, yaxis2, color2);
	graph_pointer2->Draw("P");
	multipointer->Add(graph_pointer);
	multipointer->Add(graph_pointer2);
	multipointer->Draw("AP");
	multipointer->GetXaxis()->SetTitle("Kinetic energy GeV");
	multipointer->GetYaxis()->SetTitle("Antiproton flux(1/(sr s GeV m^2))" );

}

void draw_graph(TString name, TString title,TString graph_title, int bin_numbers, axis ax, Double_t *yaxis, Float_t color) 
{
	TMultiGraph *multipointer = new TMultiGraph();
	multipointer->SetNameTitle(name, title);
	TGraph *graph_pointer = create_graph(graph_title, bin_numbers, ax, yaxis, color);
	graph_pointer->Draw("P");
	multipointer->Add(graph_pointer);
	multipointer->Draw("AP");
	multipointer->GetXaxis()->SetTitle("Kinetic energy GeV");
...
void create_canvas(TString name, TString title, TString name2, TString title2, int bin_numbers, axis ax, Double_t *yaxis, Double_t *yaxis2, Double_t *yaxis3, Float_t color, Float_t color2) 
{
	...
	gPad->SetLogx();
	gPad->SetLogy();
	draw_two_graphs(name, title,"experimental flux", "simulated flux", bin_numbers, ax, yaxis, yaxis2, color, color2);
	gPad->BuildLegend();
	canvas->Update();
	canvas->cd(2);
	gPad->SetLogx();
	draw_graph(name2, title2,"ratio", bin_numbers, ax, yaxis3, color);
	gPad->BuildLegend();
	canvas->Update();
	canvas->Modified();
...
//Definding stuff with constant value
	...
	TString path = "/nfs/mds/project/e18sat/sim_ekemar_mds/testbins4/";
	...
	const char *directory_name = "/nfs/mds/project/e18sat/sim_ekemar_mds/testbins4";
...
//Getting a list from folder	
	...
	//---------Read in Tree from file-------------------------
		...
		double the_geometrical_factor = calculate_geometrical_factor(startaltitude, detector_altitude, latitud_max,latitud_min, longitud_max, longitud_min);
		norm_factor = calculated_norm(the_solidangle ,area_conversation_factor, the_integrated_primary_flux,total_events, the_geometrical_factor);
		...
		for (float eventnumber =  0 ; eventnumber < properties->GetEntriesFast();++eventnumber)
		{
			properties->GetEntry(eventnumber);
			{
				...
				filter_pointer(latitud_min, latitud_max, longitud_min, longitud_max, secondary_latitude_vec, secondary_longitude_vec, secondary_PDG_vec, secondary_Energy_vec,
				  2212, flux_histogram, secondary_energy_antiprotons,secondary_energy_protons);
	...
	Double_t *firstratio = ratio(nbins, BESS_data_particle_flux);
	create_canvas("Flux", "flux", "Ratio", "Ratio" , nbins, xgraph, BESS_data_particle_flux, ygraph.axis_array, firstratio, 4, 3);
