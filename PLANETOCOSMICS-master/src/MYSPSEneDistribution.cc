...
///////////////////////////////////////////////////////////////////////////////
//
// MODULE:        MYSPSEneDistribution.cc
//
// Version:      1.0
// Date:         5/02/04
// Author:       Fan Lei 
...
MYSPSEneDistribution::MYSPSEneDistribution()
{
  //
...
void MYSPSEneDistribution::SetEnergyDisType(G4String DisType)
{
  ...
  if (EnergyDisType == "User"){
    ...
    if (ZeroPhysVector1) delete ZeroPhysVector1;
    ZeroPhysVector1 =new G4PhysicsOrderedFreeVector;
    ArbEnergyH = *ZeroPhysVector1 ;
    IPDFArbEnergyH = *ZeroPhysVector1 ;
    IPDFArbExist = false;
...
void MYSPSEneDistribution::SetEmin(G4double emi)
{
  Emin = emi;
}

void MYSPSEneDistribution::SetEmax(G4double ema)
{
  Emax = ema;
}

void MYSPSEneDistribution::SetMonoEnergy(G4double menergy)
{
  MonoEnergy = menergy;
}

void MYSPSEneDistribution::SetBeamSigmaInE(G4double e)
{
  SE = e;
}
void MYSPSEneDistribution::SetAlpha(G4double alp)
{
  alpha = alp;
}

void MYSPSEneDistribution::SetTemp(G4double tem)
{
  Temp = tem;
}

void MYSPSEneDistribution::SetEzero(G4double eze)
{
  Ezero = eze;
}

void MYSPSEneDistribution::SetGradient(G4double gr)
{
  grad = gr;
}

void MYSPSEneDistribution::SetInterCept(G4double c)
{
  cept = c;
}

void MYSPSEneDistribution::UserEnergyHisto(G4ThreeVector input)
{
  G4double ehi, val;
  ehi = input.x();
  val = input.y();
...
void MYSPSEneDistribution::ArbEnergyHisto(G4ThreeVector input)
{
  G4double ehi, val;
  ehi = input.x();
  val = input.y();
...
void MYSPSEneDistribution::EpnEnergyHisto(G4ThreeVector input)
{
  G4double ehi, val;
  ehi = input.x();
  val = input.y();
...
void MYSPSEneDistribution::Calculate()
{
...
void MYSPSEneDistribution::CalculateCdgSpectrum()
{
...
void MYSPSEneDistribution::CalculateBbodySpectrum()
{
...
void MYSPSEneDistribution::InputEnergySpectra(G4bool value)
{
...
void MYSPSEneDistribution::InputDifferentialSpectra(G4bool value)
{
...
void MYSPSEneDistribution::ArbInterpolate(G4String IType)
{
...
void MYSPSEneDistribution::LinearInterpolation()
{
...
void MYSPSEneDistribution::LogInterpolation()
{
  ...
  G4int i, count;
  G4int maxi = ArbEnergyH.GetVectorLength();
  G4cout<<maxi<<std::endl;
  for(i=0;i<maxi;i++) {
    Arb_x[i] = ArbEnergyH.GetLowEdgeEnergy(size_t(i));
    Arb_y[i] = ArbEnergyH(size_t(i));
    if(verbosityLevel == 2){
    	G4cout<<Arb_x[i]<<std::endl;
    	G4cout<<Arb_y[i]<<std::endl;
    }	
  }
  // Points are now in x,y arrays. If the spectrum is integral it has to be
  // made differential and if momentum it has to be made energy.
  ...
  //Adding from L. Desorgher
  //The Arb_Cum_Area[0] should alos be initialise to 0
  // Otherwise the log distributionis not always well produced as it was tghe case
  // with some  PLANETOCOMICS examples
  // The foolowing line has to be added
...
void MYSPSEneDistribution::LogInterpolation1()
{
  // Interpolation based on Logarithmic equations
  // Generate equations of line segments
  // y = Ax**alpha => log y = alpha*logx + logA
  ...
  std::vector<double> area_seg,arb_x,arb_y,arb_cum_area;
  ...
  for(i=0;i<maxi;i++) {
    ...
    arb_x.push_back(ArbEnergyH.GetLowEdgeEnergy(size_t(i)));
    arb_y.push_back(ArbEnergyH(size_t(i)));
    G4cout<<"Test LOG1" <<std::endl;
    G4cout<<arb_x[i]<<std::endl;
    G4cout<<arb_y[i]<<std::endl;
  ...
  if(DiffSpec == false) {
    ...
    for( count=0;count<maxi-1;count++) {
      arb_y[size_t(count)] = 
      	(arb_y[size_t(count)] - arb_y[size_t(count)+1])/(arb_x[size_t(count)+1]-arb_x[size_t(count)]);
  ...
  if(EnergySpec == false) {
    ...
    if(particle_definition == NULL)
      ...
      G4double total_energy;
      for(count=0;count<maxi;count++) {
	total_energy = std::sqrt((arb_x[size_t(count)]*arb_x[size_t(count)]) 
			    + (mass*mass)); // total energy
	
	arb_y[size_t(count)] = arb_y[size_t(count)] * arb_x[size_t(count)]/total_energy; 
	arb_x[size_t(count)] = total_energy - mass ;  // kinetic energy
      }
    }
  }
  //
  G4cout<<"Test LOG2" <<std::endl;
  i=1;
  Arb_alpha[0] = 0.;
  Arb_Const[0] = 0.;
  area_seg.push_back(0.);
  arb_cum_area.push_back(0.);
  if(arb_x[0] <= 0. || arb_y[0] <= 0.)
    {
      G4cout << "You should not use log interpolation with points <= 0." << G4endl;
      G4cout << "These will be changed to 1e-20, which may cause problems" << G4endl;
  ...
  while(i <maxi)
      ...
      if(arb_x[i] <= 0. || arb_y[i] <= 0.)
	{
      ...
      G4cout<<"Test LOG4" <<std::endl;
      double x1=arb_x[size_t(i-1)];
      double x2=arb_x[size_t(i)];
      double y1=arb_y[size_t(i-1)];
      double y2=arb_y[size_t(i)];	
      
      Arb_alpha[i] = std::log10(y2/y1)/std::log10(x2/x1);
      G4cout<<"Test LOG5" <<std::endl;
      Arb_Const[i] = y2/(std::pow(x2,Arb_alpha[i]));
      G4cout<<"Test LOG6" <<std::endl;
      
      alp = Arb_alpha[i] + 1;
      G4cout<<"Test LOG7" <<std::endl;
      area_seg.push_back((Arb_Const[i]/alp) * (std::pow(x2,alp) - std::pow(x1,alp)));
      G4cout<<"Test LOG8" <<std::endl;
      sum = sum + area_seg[size_t(i)];
      G4cout<<"Test LOG9" <<std::endl;
      G4double val = arb_cum_area[size_t(i-1)] + area_seg[size_t(i)];
      arb_cum_area.push_back( val);
      G4cout<<"Test LOG10" <<std::endl;
      if(verbosityLevel == 2)
	G4cout << Arb_alpha[i]<<'\t' << Arb_Const[i]<<'\t'  << area_seg[i]<<'\t'  << G4endl;
      i++;
    }
  ...
  while(i<maxi)
    { G4cout<<sum <<std::endl;
      G4cout<<arb_cum_area[size_t(i)] <<std::endl;
      arb_cum_area[size_t(i)] = arb_cum_area[size_t(i)]/sum;
      IPDFArbEnergyH.InsertValues(arb_x[size_t(i)], arb_cum_area[size_t(i)]);
      G4cout<<arb_x[size_t(i)]<<std::endl;
      G4cout<<arb_cum_area[size_t(i)] <<std::endl;
      i++;
    }
  area_seg.clear();
  ...
  arb_cum_area.clear();
  if(verbosityLevel >= 1)
    G4cout << "Leaving LogInterpolation1 " << G4endl;
}

void MYSPSEneDistribution::ExpInterpolation()
{
...
void MYSPSEneDistribution::SplineInterpolation()
{
...
void MYSPSEneDistribution::GenerateMonoEnergetic()
{
  // Method to generate MonoEnergetic particles.
  particle_energy = MonoEnergy;
}

void MYSPSEneDistribution::GenerateGaussEnergies()
{
...
void MYSPSEneDistribution::GenerateLinearEnergies(G4bool bArb = false)
{
  G4double rndm;
...
void MYSPSEneDistribution::GeneratePowEnergies(G4bool bArb = false)
{
...
void MYSPSEneDistribution::GenerateExpEnergies(G4bool bArb = false)
{
...
void MYSPSEneDistribution::GenerateBremEnergies()
{
  // Method to generate particle energies distributed according 
...
void MYSPSEneDistribution::GenerateBbodyEnergies()
{
...
void MYSPSEneDistribution::GenerateCdgEnergies()
{
...
void MYSPSEneDistribution::GenUserHistEnergies()
{
...
void MYSPSEneDistribution::GenArbPointEnergies()
{
  if(verbosityLevel > 0)
  ...
  if(IntType != "Spline")
    {
      ...
      nabove = IPDFArbEnergyH.GetVectorLength();
       //     G4cout << nbelow<<'\t'<<nabove << G4endl;
      // Binary search to find bin that rndm is in
      while(nabove-nbelow > 1)
	{
	  middle = (nabove + nbelow)/2;
	  //G4cout << middle<<'\t'<<rndm<<'\t'<<IPDFArbEnergyH(size_t(middle))<< G4endl;
...
void MYSPSEneDistribution::GenEpnHistEnergies()
{
...
void MYSPSEneDistribution::ConvertEPNToEnergy()
{
...
//
void MYSPSEneDistribution::ReSetHist(G4String atype)
{
  if (atype == "energy"){
    UDefEnergyH = IPDFEnergyH = ZeroPhysVector ;
    IPDFEnergyExist = false ;
...
G4double MYSPSEneDistribution::GenerateOne(G4ParticleDefinition* a)
{
