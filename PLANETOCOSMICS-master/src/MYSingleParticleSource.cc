...
MYSingleParticleSource::MYSingleParticleSource()
{
  ...
  biasRndm = new MYSPSRandomGenerator();
  posGenerator = new MYSPSPosDistribution();
  posGenerator->SetBiasRndm(biasRndm);
  angGenerator = new MYSPSAngDistribution();
  angGenerator->SetPosDistribution(posGenerator);
  angGenerator->SetBiasRndm(biasRndm);
  eneGenerator = new MYSPSEneDistribution();
...
void MYSingleParticleSource::SetVerbosity(int vL)
{
  verbosityLevel = vL;
...
void MYSingleParticleSource::SetParticleDefinition
...
void MYSingleParticleSource::GeneratePrimaryVertex(G4Event *evt)
{
