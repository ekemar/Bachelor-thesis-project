...
///////////////////////////////////////////////////////////////////////////////
//
// MODULE:       MYGeneralParticleSource.cc
//
// Version:      2.0
// Date:         5/02/04
// Author:       Fan Lei 
...
void MYGeneralParticleSource::AddaSource(G4double aV)
{
  currentSource = new MYSingleParticleSource();
  theMessenger->SetParticleGun(currentSource);
  sourceVector.push_back(currentSource);
...
void MYGeneralParticleSource::IntensityNormalization()
{
  G4double total  = 0.;
  size_t i = 0 ;
...
void MYGeneralParticleSource::ListSource()
{
...
void MYGeneralParticleSource::SetCurrentSourceto(G4int aV)
{
  size_t id = size_t (aV) ;
...
void MYGeneralParticleSource::SetCurrentSourceIntensity(G4double aV)
{
  sourceIntensity[currentSourceIdx] = aV;
  normalised = false;
}

void MYGeneralParticleSource::ClearAll()
{
  currentSourceIdx = -1;
  currentSource = 0;
  sourceVector.clear();
...
void MYGeneralParticleSource::DeleteaSource(G4int aV)
{
  size_t id = size_t (aV) ;
  if ( id <= sourceIntensity.size() ) {
...
void MYGeneralParticleSource::GeneratePrimaryVertex(G4Event* evt)
{
  if (!multiple_vertex){
